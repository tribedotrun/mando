...
export function useClarificationTab(taskId: number, questions: ClarifierQuestion[]) {
  ...
  } = useTextImageDraft(`clarifyImage:${taskId}:${qFingerprint}`);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    return null;
  }, [answers, unanswered, taskId, clearAnswersDraft, image, clearImageDraft, clarifyMut]);

  return {
    questions: { unanswered, answers, setAnswers, filledCount },
    mutation: { clarifyMut },
