...
import type { SidebarChild } from '#renderer/utils';
import { TaskRow, WorkbenchRow } from '#renderer/global/components/SidebarChildRows';

export type { SidebarChild } from '#renderer/utils';
...
export function SidebarProjectItem({
  ...
  const hasActiveWt =
    items.some((c) => c.kind === 'workbench' && c.wb.worktree === activeWorktreeCwd) ?? false;
  ...
  return (
    ...
    >
      ...
      {/* Expanded children: unified list of tasks and taskless workbenches,
          sorted together by last activity. */}
      {expanded && items.length > 0 && (
        <div className="flex flex-col gap-0.5 pb-1 pt-0.5">
          {items.map((child) =>
            child.kind === 'task' ? (
              <TaskRow
                key={`task:${child.task.id}`}
                task={child.task}
                onOpenTask={onOpenTask}
                onArchiveWorkbench={onArchiveWorkbench}
                onPinWorkbench={onPinWorkbench}
              />
            ) : (
              <WorkbenchRow
                key={`wb:${child.wb.id}`}
                projectName={name}
                wb={child.wb}
                activeWorktreeCwd={activeWorktreeCwd ?? null}
                renamingWbId={renamingWbId}
                setRenamingWbId={setRenamingWbId}
                onOpenWorktree={onOpenWorktree}
                onArchiveWorkbench={onArchiveWorkbench}
                onPinWorkbench={onPinWorkbench}
                onUnpinWorkbench={onUnpinWorkbench}
                onRenameWorkbench={onRenameWorkbench}
                onOpenWorkbenchInFinder={onOpenWorkbenchInFinder}
                onCopyWorkbenchPath={onCopyWorkbenchPath}
              />
            ),
          )}
        </div>
