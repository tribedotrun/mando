...
import { fmtDuration, relativeTime } from '#renderer/utils';
...
import { formatCallerLabel, buildSessionSequence } from '#renderer/components/SessionsHelpers';
...
export function SessionsTab({
  ...
  return (
    <div>
      {reversed.map((s) => {
        ...
        return (
          <button
            ...
            className="mb-1 flex w-full items-center gap-3 rounded-md px-2 py-2 text-left hover:bg-[var(--color-surface-2)]"
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-caption" style={{ color: 'var(--color-text-3)' }}>
                ...
                {s.cost_usd != null && s.cost_usd > 0 && (
                  <span> &middot; ${s.cost_usd.toFixed(2)}</span>
                )}
              </div>
            </div>
      ...
      <div
        className="mt-2 flex items-center gap-3 border-t pt-3 text-caption"
      ...
      >
        ...
        {totalCost > 0 && <span>&middot; ${totalCost.toFixed(2)}</span>}
      </div>
    </div>
  );
}

function SessionDot({ status }: { status?: string }): React.ReactElement {
  const color =
    status === 'running'
      ? 'var(--color-success)'
      : status === 'stopped'
...
/** Shorten absolute paths by replacing the home directory with ~/ */
...
function InfoSection({
  ...
  return (
    <div>
      <div
        className="mb-2 text-label font-medium uppercase tracking-widest"
...
function CollapsibleSection({
  ...
  return (
    <div>
      <button
        onClick={onToggle}
        className="mb-2 flex items-center gap-1.5 text-label font-medium uppercase tracking-widest"
        style={{
...
function CopyValue({ value, display }: { value: string; display?: string }): React.ReactElement {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1.5 text-left text-code hover:opacity-80"
      style={{
