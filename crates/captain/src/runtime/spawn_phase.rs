...
///
/// This handles `ActionKind::Ship` actions from the tick-summary action
/// contract. In the current captain flow, no production code path emits
/// `ActionKind::Ship` — ships are driven by `captain_review` returning
/// `action == "ship"`, which goes through `apply_verdict` (which does
/// populate the `awaiting_review` event with confidence fields). This
/// branch remains because the action contract supports it and tests
/// exercise it; if it ever re-enters production it will write an
/// `awaiting_review` event without confidence, which the mergeability
/// tick treats as "not high" and leaves for human review.
async fn execute_ship(
    ...
    let pid =
        crate::io::pid_lookup::resolve_pid(cc_sid, &action.worker).unwrap_or(crate::Pid::new(0));
    if pid.as_u32() > 0 {
    ...
    if let Some(wt) = &it.worktree {
        ...
        if let Some((_, project_config)) =
            settings::config::resolve_project_config(Some(&it.project), config)
        {
    ...
    let pr_label_str = it.pr_number.map(crate::pr_label);
    let event = crate::TimelineEvent {
        event_type: crate::TimelineEventType::AwaitingReview,
