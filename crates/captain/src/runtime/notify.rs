...
use global_types::events::{NotificationKind, NotificationPayload};
use global_types::notify::NotifyLevel;
...
impl Notifier {
    ...
    pub async fn check_rate_limit(
        ...
        let account_suffix = match credential_id {
            Some(id) => match settings::io::credentials::labels_by_ids(pool, &[id]).await {
...
mod tests {
    ...
    async fn check_rate_limit_includes_credential_label() {
        let pool = test_pool().await;
        let cred_id = settings::io::credentials::insert(&pool, "team-alpha", "tok_test", None)
            .await
            .unwrap();
