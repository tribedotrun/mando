...
pub async fn add_task(
    ...
    let (project_id, project_name) = if let Some(ref name) = resolved_project {
        ...
        let id = settings::io::projects::upsert(store.pool(), name, path, github_repo).await?;
        (id, name.clone())
    } else {
    ...
    let _ = super::timeline_emit::emit(
        store.pool(),
        id,
        crate::TimelineEventType::Created,
        "captain",
...
pub async fn trigger_captain_tick(
    ...
    .await?;
    Ok(crate::service::tick_summary::tick_result_to_json(&result))
}
...
pub async fn cancel_item(store: &TaskStore, id: i64, pool: &sqlx::SqlitePool) -> Result<()> {
    ...
    let running = sessions::io::queries::list_running_sessions_for_task(pool, id)
        .await
...
pub async fn validate_rate_limited_task(store: &TaskStore, id: i64) -> Result<()> {
    ...
    let credential_blocking =
        settings::io::credentials::earliest_cooldown_remaining_secs(pool).await > 0;
    anyhow::ensure!(
    ...
    if credential_blocking {
        if let Err(e) = settings::io::credentials::clear_all_cooldowns(pool).await {
            tracing::warn!(
                module = "captain",
                task_id = id,
                error = %e,
