...
pub(crate) async fn reconcile_running_sessions(
    ...
    let running = match sessions::io::queries::list_running_sessions(pool).await {
        Ok(rows) => rows,
        Err(e) => {
    ...
    for row in &running {
        ...
        if let Some(pid) = crate::io::pid_registry::get_verified_pid(sid) {
            if pid.as_u32() > 0 {
                if global_claude::is_process_alive(pid) {
                    ...
                    if global_claude::has_rate_limit_rejection(&stream_path).is_some()
        ...
        if let Some(result) = global_claude::get_stream_result(&stream_path) {
            let status = if global_claude::is_clean_result(&result) {
    ...
    for job in &jobs {
        let stream_path = global_infra::paths::stream_path_for_session(&job.session_id);
        if let Some(rl) = global_claude::last_rate_limit_status(&stream_path) {
            let cred_id = sessions::io::queries::get_credential_id(pool, &job.session_id)
                .await
                .unwrap_or(None);
            tracing::info!(
                module = "captain",
