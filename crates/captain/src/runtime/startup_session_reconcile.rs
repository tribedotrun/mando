...
pub async fn reconcile_startup_sessions(pool: &SqlitePool) -> Result<()> {
    let running = sessions::io::queries::list_running_sessions(pool)
        .await
...
mod tests {
    use super::*;
    use global_db::Db;
    use sessions::io::queries::{upsert_session, SessionRow, SessionUpsert};
    ...
    async fn load_session(pool: &SqlitePool, sid: &str) -> SessionRow {
        sessions::io::queries::session_by_id(pool, sid)
            .await
            .unwrap()
            .unwrap()
    }
