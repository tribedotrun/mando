...
pub(super) async fn reap_dead_rebase_workers(items: &mut [Task], pool: &sqlx::SqlitePool) {
    for item in items.iter_mut() {
        ...
        let found_sid = match sessions::io::queries::find_session_id_by_worker_name(pool, &rw).await
        {
            Ok(Some(sid)) => {
...
mod tests {
    ...
    async fn reap_defers_when_stream_cost_missing() {
        ...
        let sid = sessions::io::queries::find_session_id_by_worker_name(&pool, worker_name)
            .await
            .unwrap();
        assert_eq!(
            sid.as_deref(),
        ...
        let row = sessions::io::queries::session_by_id(&pool, &session_id)
            .await
            .unwrap()
