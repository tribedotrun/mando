...
pub async fn activate(
    ...
    match settings::io::credentials::set_rate_limit_cooldown(pool, credential_id, until as i64)
        .await
    {
        Ok(_) => {
            tracing::warn!(
...
pub async fn clear(pool: &SqlitePool, credential_id: i64) {
    match settings::io::credentials::set_rate_limit_cooldown(pool, credential_id, 0).await {
        Ok(_) => {
            tracing::info!(
                module = "captain",
                credential_id,
...
pub async fn check_and_activate_from_stream(pool: &SqlitePool, session_id: &str) -> bool {
    ...
    match global_claude::has_rate_limit_rejection(&stream_path) {
        Some(rej) => {
            ...
            let cred_id = sessions::io::queries::get_credential_id(pool, session_id)
                .await
                .unwrap_or(None);
