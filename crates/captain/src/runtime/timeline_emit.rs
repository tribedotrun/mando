...
pub(crate) async fn emit(
    ...
    for attempt in 1..=MAX_ATTEMPTS {
        match crate::io::queries::timeline::append(pool, task_id, &event).await {
            Ok(()) => return Ok(()),
            Err(e) => {
...
async fn task_rate_limit_remaining_secs(item: &crate::Task, pool: &SqlitePool) -> u64 {
    let cred_id = match item.session_ids.worker.as_deref() {
        Some(sid) => sessions::io::queries::get_credential_id(pool, sid)
            .await
            .unwrap_or(None),
        None => None,
    };
    match cred_id {
        Some(cid) => settings::io::credentials::cooldown_remaining_secs(pool, cid)
            .await
            .max(0) as u64,
