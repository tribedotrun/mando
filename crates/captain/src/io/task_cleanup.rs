...
pub(crate) async fn cleanup_task(
    ...
    let session_ids: Vec<String> = sessions::io::queries::list_sessions_for_task(pool, item.id)
        .await
        .unwrap_or_default()
        .into_iter()
        .map(|s| s.session_id)
    ...
    {
        match sessions::io::queries::delete_sessions_for_task(pool, item.id).await {
            Ok(n) => {
                if n > 0 {
