...
pub async fn terminate_session(
    ...
    match sessions::io::queries::is_session_running(pool, session_id).await {
        Ok(false) => return,
        Err(e) => {
            tracing::warn!(
    ...
    let db_ok = match sessions::io::queries::update_session_status_with_cost(
        pool,
        session_id,
        new_status,
        cost_usd,
    ...
    if let Some(hs) = health_state {
        if let Ok(Some(row)) = sessions::io::queries::session_by_id(pool, session_id).await {
