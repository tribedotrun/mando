...
use crate::{RebaseState, Task, TaskRouting, TaskUpdateError};
use anyhow::{Context, Result};
use global_types::session::SessionEntry;
use sessions::io::queries as session_queries;
use sqlx::SqlitePool;
...
impl TaskStore {
    ...
    pub async fn upsert_session(&self, entry: &SessionEntry) -> Result<()> {
        let status: global_types::SessionStatus = entry.status.parse().map_err(|e| {
            anyhow::anyhow!(
                "upsert_session: invalid session status {:?}: {e}",
                entry.status
            )
        })?;
        session_queries::upsert_session(
            &self.pool,
            &session_queries::SessionUpsert {
...
mod tests {
    ...
    async fn test_store() -> TaskStore {
        ...
        settings::io::projects::upsert(db.pool(), "test", "", None)
            .await
            .unwrap();
