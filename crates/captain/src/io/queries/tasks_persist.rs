...
/// Set the PR number on a task after draft PR creation.
...
/// Revert InProgress planning tasks to Queued. Used on startup to clean up
/// orphaned planning pipelines that were killed when the daemon exited.
pub async fn revert_orphaned_planning(pool: &SqlitePool) -> Result<u64> {
    let result = sqlx::query(
        "UPDATE tasks SET status = 'queued', worker = NULL, session_ids = '{}', \
         worker_started_at = NULL, last_activity_at = ? \
         WHERE status = 'in-progress' AND planning = 1",
    )
