...
pub fn row_to_config(row: &ProjectRow) -> mando_config::settings::ProjectConfig {
...
pub fn config_to_row(pc: &mando_config::settings::ProjectConfig) -> ProjectRow {
    ProjectRow {
        id: 0,
        name: pc.name.clone(),
        path: pc.path.clone(),
...
pub async fn startup_sync(
    ...
    if mando_config::backfill_project_logos(config) {
        for pc in config.captain.projects.values() {
            if pc.logo.is_some() {
                let row = config_to_row(pc);
                let _ = upsert_full(pool, &row).await;
            }
        }
    }
    Ok(())
