...
) -> Result<ScoutAskWithImages, (StatusCode, Json<Value>)> {
...
pub(crate) async fn extract_nudge(
    request: axum::extract::Request,
) -> Result<NudgeWithImages, (StatusCode, Json<Value>)> {
    if is_multipart(&request) {
...
pub(crate) async fn extract_clarify(
    request: axum::extract::Request,
) -> Result<ClarifyWithImages, (StatusCode, Json<Value>)> {
    if is_multipart(&request) {
