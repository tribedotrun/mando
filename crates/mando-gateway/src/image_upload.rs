...
use crate::response::{error_response, internal_error_with};
...
pub(crate) async fn cleanup_saved_images(filenames: &[String]) {
    ...
    for name in filenames {
        ...
        if let Err(e) = tokio::fs::remove_file(&path).await {
            tracing::warn!(file = %path.display(), error = %e, "failed to clean up orphaned image");
        }
    }
}
...
pub(crate) async fn into_multipart(
    request: axum::extract::Request,
) -> Result<Multipart, (StatusCode, Json<Value>)> {
    <Multipart as FromRequest<()>>::from_request(request, &())
        .await
        .map_err(|e| error_response(StatusCode::BAD_REQUEST, &e.to_string()))
}

pub(crate) async fn field_text(field: Field<'_>) -> Result<String, (StatusCode, Json<Value>)> {
    field
        .text()
        .await
        .map_err(|e| error_response(StatusCode::BAD_REQUEST, &e.to_string()))
...
pub(crate) async fn field_id(field: Field<'_>) -> Result<i64, (StatusCode, Json<Value>)> {
...
pub(crate) async fn extract_feedback(
    request: axum::extract::Request,
) -> Result<FeedbackWithImages, (StatusCode, Json<Value>)> {
...
pub(crate) async fn extract_ask(
    request: axum::extract::Request,
) -> Result<AskWithImages, (StatusCode, Json<Value>)> {
    if is_multipart(&request) {
...
async fn extract_ask_multipart(
    request: axum::extract::Request,
) -> Result<AskWithImages, (StatusCode, Json<Value>)> {
    let mut mp = into_multipart(request).await?;
    let mut id: Option<i64> = None;
