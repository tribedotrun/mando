...
mod tests {
    ...
    fn captain_review_render_includes_confidence_rubric() {
        ...
        // UI evidence rule requires inspectable recording.
        assert!(
            rendered.contains("screenshot AND an inspectable recording"),
            "missing UI screenshot+recording rule"
        );
        // Non-viewable MP4/MOV/WebM recordings are not evidence. Match on
        // "unverified" alone since the YAML wraps "unverified claim" across
        // lines which stay wrapped in the render.
        assert!(
            rendered.contains("unverified"),
            "missing caption-is-not-evidence rule"
        );
