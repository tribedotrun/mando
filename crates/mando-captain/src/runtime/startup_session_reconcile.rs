...
pub async fn reconcile_startup_sessions(pool: &SqlitePool) -> Result<()> {
    let running = mando_db::queries::sessions::list_running_sessions(pool)
        .await
    ...
    for row in &running {
        ...
        let status = match mando_cc::get_stream_result(&stream_path) {
            Some(result) if mando_cc::is_clean_result(&result) => {
                salvaged += 1;
...
mod tests {
    use super::*;
    use mando_db::queries::sessions::{upsert_session, SessionRow, SessionUpsert};
    use mando_db::Db;
    ...
    async fn load_session(pool: &SqlitePool, sid: &str) -> SessionRow {
        mando_db::queries::sessions::session_by_id(pool, sid)
            .await
            .unwrap()
            .unwrap()
    }

    /// Shared test lock: tests mutate the process-wide `MANDO_DATA_DIR`
    /// env var, which races with any other test that does the same.
    /// Serializing all tests in this module under one async mutex avoids
    /// that; a plain `std::sync::Mutex` would trigger the
    /// `await_holding_lock` clippy warning because each test awaits while
    /// holding the guard.
    ...
    fn write_stream(session_id: &str, content: &str) {
        let path = mando_config::stream_path_for_session(session_id);
