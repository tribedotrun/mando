...
pub(crate) async fn handle_summary(
    ...
    let result = client
        ...
        .await?;
    let artifact_id = result["artifact_id"].as_i64().unwrap_or(0);
...
pub(crate) async fn handle_evidence(files: &[String], captions: &[String]) -> anyhow::Result<()> {
    ...
    let file_inputs: Vec<serde_json::Value> = files
        .iter()
        .zip(captions.iter())
        .map(|(path, caption)| {
            ...
            let ext = p
                ...
                .to_string_lossy()
                .to_string();
            json!({"filename": filename, "ext": ext, "caption": caption})
        })
        .collect();
    ...
    let result = client
        .post(
            ...
            &json!({"files": file_inputs}),
        )
        .await?;
    let artifact_id = result["artifact_id"].as_i64().unwrap_or(0);
    let media = result["media"].as_array();
    ...
    for (i, source_path) in files.iter().enumerate() {
        let local_path = media
            .and_then(|m| m.get(i))
            .and_then(|m| m["local_path"].as_str())
            .unwrap_or("");
