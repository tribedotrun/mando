...
/// already-completed credential swap, so it's surfaced as a warning rather
/// than a hard error.
#[cfg(target_os = "macos")]
pub(crate) async fn relaunch_chatgpt_app() {
        ...
        .await
    {
        ...
        Ok(status) => {
            eprintln!("warning: `open -a ChatGPT` exited with {status}; relaunch it manually");
        }
        Err(err) => {
            eprintln!("warning: failed to relaunch ChatGPT.app: {err}; relaunch it manually");
        }
    }
}
...
/// Warn (stderr only, never kill) about Codex-related processes running
...
#[cfg(target_os = "macos")]
pub(crate) async fn warn_external_codex_processes() {
    match external_codex_processes().await {
        ...
        Ok(_) => {}
        Err(err) => {
            eprintln!("warning: failed to check for external Codex processes: {err}");
        }
    }
}
...
/// bundle nor this `mando` process itself (whose own argv contains the
/// `codex` subcommand name and would otherwise false-positive).
#[cfg(target_os = "macos")]
async fn external_codex_processes() -> Result<Vec<String>> {
    ...
    Ok(text
        ...
        .filter_map(|line| {
            ...
            if pid == own_pid || cmd.contains(CHATGPT_BUNDLE_MARKER) {
                None
            } else {
                Some(format!("{pid} {cmd}"))
            }
