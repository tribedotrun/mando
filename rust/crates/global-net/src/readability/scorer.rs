...
pub(crate) fn score(dom: &Dom) -> HashMap<NodeId, f64> {
    ...
    for id in scored_ids {
        ...
        if modifier != 0.0 {
            // `id` was drawn from `scored_ids` which is a snapshot of
            // `scores.keys()`, so the entry exists by construction. Use
            // an entry call so any future refactor that breaks the
            // invariant still produces a well-defined value instead of
            // panicking silently.
            *scores.entry(id).or_insert(0.0) += modifier;
        }
    }
...
mod tests {
    ...
    fn paragraph_scores_propagate_to_parent() {
        ...
        let dom = parse(html);
        crate::readability::cleaner::clean(&dom);
