...
use crate::io::ShellEnvResolver;
use crate::io::{TerminalHistoryMeta, TerminalHistoryStore};
use crate::types::{
...
pub struct TerminalSession {
    ...
    size: std::sync::Mutex<TerminalSize>,
...
impl TerminalSession {
    pub fn spawn(
        ...
        // The SessionStart hook in ~/.claude/settings.json reads MANDO_TERMINAL_ID
        // to POST the Claude Code session UUID back to the daemon. Always set it to
    ...
    pub fn restored(meta: TerminalHistoryMeta, history: Arc<TerminalHistoryStore>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            size: std::sync::Mutex::new(meta.size),
            live: None,
            history,
        })
    }
    ...
    pub fn resize(&self, size: TerminalSize) -> anyhow::Result<()> {
        ...
        let master = live.master.lock().unwrap_or_else(|p| p.into_inner());
        master.resize(pty_size(size))?;
        *self.size.lock().unwrap_or_else(|p| p.into_inner()) = size;
        if let Err(err) = self.history.update_size(&self.id, size) {
            warn!(session = %self.id, error = %err, "failed to persist terminal size");
        }
        Ok(())
    }
    ...
    pub fn kill(&self) -> anyhow::Result<()> {
        if let Some(live) = &self.live {
            let mut killer = live.killer.lock().unwrap_or_else(|p| p.into_inner());
            killer.kill()?;
        }
        Ok(())
    }
    ...
    /// Update the captured Claude Code session id for this terminal, both
    /// in-memory and on disk. Called by the SessionStart hook callback.
    pub fn set_cc_session_id(&self, cc_session_id: String) -> anyhow::Result<()> {
        self.history
            .set_cc_session_id(&self.id, cc_session_id.clone())?;
        *self.cc_session_id.lock().unwrap_or_else(|p| p.into_inner()) = Some(cc_session_id);
        Ok(())
    }
}
...
fn spawn_waiter_thread(ctx: WaiterThreadCtx) -> anyhow::Result<()> {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let Some(mut child) = child_guard.child.take() else {
                warn!(
                    module = "terminal-session",
                    session = session_id,
                    "pty wait thread: child handle already consumed — skipping"
                );
                return;
            };
            ...
            let ended = global_types::now_rfc3339();
            *exit_code.lock().unwrap_or_else(|p| p.into_inner()) = code;
            *ended_at.lock().unwrap_or_else(|p| p.into_inner()) = Some(ended.clone());
            *state.lock().unwrap_or_else(|p| p.into_inner()) = SessionState::Exited;
