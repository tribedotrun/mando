...
use crate::io::TerminalHistoryStore;
use crate::runtime::session::TerminalSession;
    ...
    pub fn create(&self, req: CreateRequest) -> anyhow::Result<Arc<TerminalSession>> {
        ...
        if sessions.len() >= Self::MAX_SESSIONS {
            global_infra::best_effort!(
                session.kill(),
                "terminal session limit cleanup: kill pending session",
            );
            global_infra::best_effort!(
                session.delete_history(),
                "terminal session limit cleanup: delete history",
            );
            anyhow::bail!(
                "terminal session limit reached ({}/{})",
                sessions.len(),
    ...
    pub fn remove(&self, id: &str) -> Option<Arc<TerminalSession>> {
        ...
        if let Some(ref session) = session {
            if session.is_running() {
                global_infra::best_effort!(session.kill(), "terminal remove() kill");
            }
