...
pub(crate) struct BatcherThreadCtx {
...
pub(crate) fn terminal_env(req: &CreateRequest) -> HashMap<String, String> {
...
pub(crate) fn pty_size(size: TerminalSize) -> PtySize {
    PtySize {
        rows: size.rows,
        cols: size.cols,
        pixel_width: 0,
...
pub(crate) fn spawn_batcher_thread(ctx: BatcherThreadCtx) -> anyhow::Result<()> {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let code = match exit_code.lock() {
                Ok(guard) => *guard,
                Err(poisoned) => {
                    tracing::warn!(
                        target: "mando-terminal",
                        module = "terminal",
                        "exit_code mutex poisoned; recovering last value",
                    );
                    *poisoned.into_inner()
                }
            };
            // Broadcast send errors indicate the receiver side was dropped
            // during shutdown. The subscriber list being empty is expected
            // and carries no recovery action — swallow quietly.
            match output_tx.send(TerminalEvent::Exit { code }) {
                Ok(_) | Err(_) => {}
            }
        })?;
    Ok(())
...
fn flush_output(
    ...
    {
        let mut buf = match output_buf.lock() {
            Ok(guard) => guard,
            Err(poisoned) => {
                tracing::warn!(
                    target: "mando-terminal",
                    module = "terminal",
                    "output_buf mutex poisoned; recovering inner state",
                );
                poisoned.into_inner()
            }
        };
    ...
    match output_tx.send(TerminalEvent::Output(data.to_vec())) {
        Ok(_) | Err(_) => {}
    }
}
