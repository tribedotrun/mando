    ...
    fn base_env(&self) -> HashMap<String, String> {
        self.cached
            .get_or_init(|| match snapshot_login_shell_env() {
                Ok(env) => env,
                Err(err) => {
                    warn!(module = "terminal", error = %err, "failed to resolve shell-derived terminal env; falling back to current process env");
...
fn run_probe_command_with_timeout(
    ...
    let stdout_thread = std::thread::spawn(move || {
        ...
        if let Some(mut pipe) = stdout_pipe {
            global_infra::best_effort!(pipe.read_to_end(&mut buf), "shell env probe stdout drain");
        }
        buf
    });
    let stderr_thread = std::thread::spawn(move || {
        ...
        if let Some(mut pipe) = stderr_pipe {
            global_infra::best_effort!(pipe.read_to_end(&mut buf), "shell env probe stderr drain");
        }
        buf
    });
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            global_infra::best_effort!(child.kill(), "shell env probe timeout kill");
            global_infra::best_effort!(child.wait(), "shell env probe timeout wait");
