...
pub async fn replace_codex(
    ...
    let result = sqlx::query(
        "UPDATE credentials
         SET label = ?1, access_token = ?2, refresh_token = ?3, id_token = ?4,
             account_id = ?5, plan_type = ?6, expires_at = ?7,
             token_updated_at = ?8, rate_limit_cooldown_until = NULL,
             updated_at = datetime('now')
         WHERE id = ?9 AND provider = 'codex'",
    )
    .bind(label)
    .bind(access_token)
    .bind(refresh_token)
...
pub async fn insert_codex(
    ...
    let id: i64 = sqlx::query_scalar(
        "INSERT INTO credentials (
            ...
            plan_type, provider, expires_at, token_updated_at, updated_at
         )
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, 'codex', ?7, ?8, datetime('now'))
         RETURNING id",
    )
    .bind(label)
    .bind(access_token)
...
/// refresh response sometimes omits it; in that case we clear the stored
/// short-lived JWT so future refresh decisions fall back to Codex's file
/// `last_refresh`/reactive 401 behavior instead of repeatedly refreshing
/// because of a stale JWT exp claim.
pub async fn update_codex_tokens(
    ...
    let result = match id_token {
            ...
            sqlx::query(
                "UPDATE credentials
                 SET access_token = ?1, refresh_token = ?2, id_token = ?3,
                     expires_at = ?4, token_updated_at = ?5, updated_at = datetime('now')
        ...
        None => {
            sqlx::query(
                "UPDATE credentials
                 SET access_token = ?1, refresh_token = ?2, id_token = NULL,
                     expires_at = ?3, token_updated_at = ?4, updated_at = datetime('now')
...
mod tests {
    ...
    #[test]
    fn parse_allows_missing_id_token_when_account_id_present() {
        let content = r#"{"auth_mode":"chatgpt","tokens":{"access_token":"AT","refresh_token":"RT","account_id":"acct-1"}}"#;
        let parsed = parse_auth_json(content).unwrap();
    ...
    #[test]
    fn serialize_without_id_token() {
        let serialized = serialize_auth_json("AT", "RT", None, Some("acct-1"), None).unwrap();
        let parsed = parse_auth_json(&serialized).unwrap();
        assert_eq!(parsed.access_token, "AT");
        assert_eq!(parsed.refresh_token, "RT");
        assert_eq!(parsed.id_token, None);
