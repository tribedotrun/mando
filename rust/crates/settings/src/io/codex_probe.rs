...
mod tests {
    ...
    fn body_with(primary_pct: f64, secondary_pct: f64, limit_reached: bool) -> UsageResponse {
        UsageResponse {
            ...
            rate_limit: RateLimitBlock {
                ...
                secondary_window: Some(WindowBlock {
    ...
    #[test]
    fn parse_limit_reached_picks_higher_util_window() {
        let outcome = parse_outcome(body_with(50.0, 99.5, true)).unwrap();
        assert_eq!(outcome.snapshot.unified_status, RateLimitStatus::Rejected);
        assert_eq!(
            outcome.snapshot.representative_claim.as_deref(),
            Some("seven_day")
    ...
    #[test]
    fn parse_credits_without_balance() {
        let json = serde_json::json!({
            "plan_type": "pro",
            "rate_limit": {
                "primary_window": { "used_percent": 4.0, "reset_at": 1 },
                "secondary_window": { "used_percent": 22.0, "reset_at": 2 },
                "limit_reached": false
            },
            "credits": { "unlimited": true }
        });
        let body: UsageResponse = serde_json::from_value(json).unwrap();
        let outcome = parse_outcome(body).unwrap();
        assert!(outcome.credits_unlimited);
        assert_eq!(outcome.credits_balance, None);
    }

    #[test]
    fn parse_numeric_credits_balance() {
        let json = serde_json::json!({
            "plan_type": "pro",
            "rate_limit": {
                "primary_window": { "used_percent": 4.0, "reset_at": 1 },
        ...
        let body: UsageResponse = serde_json::from_value(json).unwrap();
        let outcome = parse_outcome(body).unwrap();
        assert_eq!(outcome.credits_balance.as_deref(), Some("42"));
    }

    #[test]
    fn parse_window_without_reset_timestamps() {
        ...
        let outcome = parse_outcome(body).unwrap();
        assert_eq!(outcome.snapshot.five_hour.status, RateLimitStatus::Rejected);
        assert!(
            outcome.snapshot.five_hour.reset_at <= outcome.snapshot.probed_at + 600,
            "missing reset must not pin cooldown centuries ahead"
        );
    }
    ...
    fn parse_single_window_response() {
        let json = serde_json::json!({
            "plan_type": "plus",
            "rate_limit": {
                "primary_window": { "used_percent": 88.0, "reset_after_seconds": 300 },
                "limit_reached": false
            }
        });
        let body: UsageResponse = serde_json::from_value(json).unwrap();
        let outcome = parse_outcome(body).unwrap();
        assert_eq!(outcome.plan_type.as_deref(), Some("plus"));
        assert_eq!(
            outcome.snapshot.unified_status,
            RateLimitStatus::AllowedWarning
        );
        assert!((outcome.snapshot.seven_day.utilization - 0.88).abs() < 1e-9);
    }
