...
use super::paths::{match_project_by_prefix, resolve_project_config};
...
fn parse_github_slug_formats() {
    use super::paths::parse_github_slug;
    assert_eq!(
...
fn load_missing_config_returns_default() {
    use std::path::Path;
    let cfg =
        crate::io::config_fs::load_config(Some(Path::new("/nonexistent/config.json"))).unwrap();
...
fn load_save_roundtrip() {
    ...
    crate::io::config_fs::save_config(&cfg, Some(&path)).unwrap();
    assert!(path.exists());
