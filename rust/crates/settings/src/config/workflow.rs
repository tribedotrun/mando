...
impl CaptainWorkflow {
    ...
    pub fn compiled_default() -> Self {
        match serde_yaml::from_str(DEFAULT_CAPTAIN_WORKFLOW) {
            Ok(v) => v,
            Err(e) => global_infra::unrecoverable!(
                "compiled captain-workflow.yaml is malformed — build defect",
                e
            ),
        }
    }
...
pub struct AgentConfig {
    ...
    /// `CcOneShot::run_with_retry` from callers that can safely ride
    /// out intermittent rate-limit or overload responses (clarifier tick,
    /// worker/review/merge spawn, scout ops). The HTTP clarify route
    /// stays non-retrying because the user is waiting on a response.
...
// crate::io::config_fs. The pure parsing entry point is parse_captain_workflow_or_default.

/// Parse captain workflow from optional override content, falling back to compiled default.
pub fn parse_captain_workflow_or_default(
    override_content: Option<&str>,
    override_path: &Path,
) -> Result<CaptainWorkflow, ConfigError> {
    match override_content {
        Some(contents) => parse_captain_workflow(contents, override_path),
        None => Ok(CaptainWorkflow::compiled_default()),
    }
}

/// Parse scout workflow from optional override content, inject config, and validate.
pub fn parse_scout_workflow_or_default(
    override_content: Option<&str>,
    override_path: &Path,
    config: &crate::config::Config,
) -> Result<ScoutWorkflow, ConfigError> {
    let mut wf = match override_content {
        Some(contents) => parse_scout_workflow(contents, override_path)?,
        None => ScoutWorkflow::compiled_default(),
    };
    inject_config_into_workflow(&mut wf, config);
    crate::config::workflow_validate::validate_scout_workflow(&wf);
    Ok(wf)
}
...
fn inject_config_into_workflow(wf: &mut ScoutWorkflow, config: &crate::config::Config) {
    let dc = &config.scout;
