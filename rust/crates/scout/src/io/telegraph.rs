...
pub fn invalidate_cache(id: i64) {
    ...
    if let Err(e) = std::fs::remove_file(&path) {
        if e.kind() != std::io::ErrorKind::NotFound {
            tracing::warn!(module = "scout-io-telegraph", id, %e, path = %path.display(), "telegraph: failed to invalidate cache");
        }
    }
}
...
fn read_cache(id: i64) -> Option<TelegraphCache> {
    ...
    match serde_json::from_str::<TelegraphCache>(&text) {
        Ok(cache) => Some(cache),
        Err(e) => {
            tracing::warn!(module = "scout-io-telegraph", id, %e, "telegraph: corrupt cache file, ignoring");
            None
        }
    }
}
...
pub async fn publish_article(id: i64, title: &str, article_md: &str) -> Result<String> {
    ...
    tracing::info!(module = "scout-io-telegraph", id, %url, "telegraph: published");
    Ok(url)
}
...
async fn ensure_token() -> Result<String> {
    ...
    tracing::info!(
        module = "scout-io-telegraph",
        "telegraph: creating new account"
    );
...
fn shared_client() -> std::sync::Arc<reqwest::Client> {
    global_net::http::telegraph_client()
}
