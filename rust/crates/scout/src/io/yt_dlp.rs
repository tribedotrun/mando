...
fn bin_path() -> PathBuf {
    global_infra::paths::bin_dir().join("yt-dlp")
}

fn version_path() -> PathBuf {
    global_infra::paths::bin_dir().join(".yt-dlp.version")
}
...
async fn download_and_verify() -> Result<()> {
    let bin_dir = global_infra::paths::bin_dir();
    tokio::fs::create_dir_all(&bin_dir).await?;

    let url = download_url();
    info!(%url, "downloading yt-dlp {VERSION}");

    let client = global_net::http::yt_dlp_client();
