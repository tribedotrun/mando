...
pub async fn inject_request_id(
    ...
    async move {
        ...
        tracing::info!(
            module = "transport-http-middleware-request_id",
            status,
            duration_ms,
            "request completed"
        );
        // UUID v4 strings are ASCII and always legal as header values.
        // Skip the header on the unreachable failure path rather than
        // panic — downstream clients can still correlate by body.
