...
fn token_path() -> PathBuf {
    global_infra::paths::data_dir().join("auth-token")
}
...
pub fn ensure_auth_token() -> String {
    ...
    let token = global_infra::uuid::Uuid::v4().to_string();

    if let Some(parent) = path.parent() {
        if let Err(e) = fs::create_dir_all(parent) {
            global_infra::unrecoverable!("failed to create auth-token parent directory", e);
        }
    }
    if let Err(e) = fs::write(&path, &token) {
        global_infra::unrecoverable!("failed to write auth-token file", e);
    }
    ...
    #[cfg(unix)]
    {
        ...
        if let Err(e) = fs::set_permissions(&path, fs::Permissions::from_mode(0o600)) {
            global_infra::unrecoverable!("failed to set auth-token file permissions to 0600", e);
        }
    }
...
fn expected_token() -> Result<String, axum::response::Response> {
    let expected = match read_auth_token() {
        ...
        None => {
            tracing::error!(
                module = "transport-http-auth",
                "auth-token file missing or unreadable — rejecting request"
            );
