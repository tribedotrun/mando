...
mod tests {
    ...
    fn classify_default_test_file() {
        assert_eq!(
            classify_file("rust/crates/mando-captain/src/biz/triage_tests.rs", &[]),
            "test"
        );
    }
    ...
    fn classify_default_nested_test_file() {
        assert_eq!(
            classify_file(
                "rust/crates/mando-captain/src/biz/tests/integration.rs",
                &[]
            ),
            "test"
        );
    ...
    fn classify_default_prod_file() {
        assert_eq!(
            classify_file("rust/crates/mando-captain/src/biz/triage.rs", &[]),
            "prod"
        );
    }
    ...
    fn classify_default_config() {
        assert_eq!(classify_file("devtools/scripts/lint.sh", &[]), "config");
    }

    #[test]
    fn classify_default_root_cargo_lock_as_config() {
        assert_eq!(classify_file("Cargo.lock", &[]), "config");
    }

    #[test]
    fn classify_default_rust_cargo_lock_as_config() {
        assert_eq!(classify_file("rust/Cargo.lock", &[]), "config");
    }
    ...
    fn classify_custom_rules_override_defaults() {
        ...
        // "devtools/scripts/lint.sh" would match default "config" but not custom rules.
        assert_eq!(classify_file("devtools/scripts/lint.sh", &rules), "prod");
    ...
    fn classify_mando_project_rules() {
        ...
        let rules = vec![
            ClassifyRule {
                category: "test".into(),
                patterns: vec![
                    "rust/crates/**/tests/**".into(),
                    "rust/crates/**/tests/*".into(),
                    "rust/crates/**/*_tests.rs".into(),
                    "rust/crates/**/tests.rs".into(),
                    "rust/cli/**/tests/**".into(),
                    "rust/cli/**/tests/*".into(),
                    "electron/tests/**".into(),
                ],
        ...
        // Rust tests
        assert_eq!(
            classify_file("rust/crates/mando-config/src/tests.rs", &rules),
            "test"
        );
        assert_eq!(
            classify_file(
                "rust/crates/mando-captain/src/biz/deterministic_tests.rs",
                &rules
            ),
            "test"
        );
        ...
        // Prod (unmatched)
        assert_eq!(
            classify_file("rust/crates/mando-gateway/src/server.rs", &rules),
            "prod"
        );
        assert_eq!(
    ...
    fn build_item_classifies_files() {
        let files = vec![
            "rust/crates/mando-captain/src/biz/triage_tests.rs".to_string(),
            "rust/crates/mando-captain/src/biz/triage.rs".to_string(),
            "README.md".to_string(),
        ];
