...
use crate::{
    io::task_store::TaskSnapshotJson, service::lifecycle::infer_transition_command, Task,
    TaskRouting,
};
...
pub async fn update_task(pool: &SqlitePool, task: &Task) -> Result<bool> {
    let mut tx = pool.begin().await?;
    let Some(current) = find_by_id_exec(&mut *tx, task.id).await? else {
        tx.rollback().await?;
        return Ok(false);
    };
    let (updated, transition_id) = update_task_exec(&mut tx, &current, task).await?;
    if !updated {
        tx.rollback().await?;
        return Ok(false);
    }
...
/// Uses SQLite's `DATE('now', 'localtime')` so all date-boundary math matches.
pub async fn today_localtime_iso(pool: &SqlitePool) -> Result<String> {
...
pub async fn daily_merge_counts(pool: &SqlitePool, days: u32) -> Result<Vec<(String, i64)>> {
    let rows: Vec<(String, i64)> = sqlx::query_as(
        "SELECT DATE(t.last_activity_at, 'localtime') AS day, COUNT(*) \
         FROM tasks t \
         WHERE t.status = 'merged' \
           AND DATE(t.last_activity_at, 'localtime') >= DATE('now', 'localtime', '-' || ? || ' days') \
         GROUP BY day \
         ORDER BY day",
    )
    .bind(days)
...
pub async fn merge_changed_items(
    ...
    changed_items: &[Task],
    merge_fn: impl Fn(&TaskSnapshotJson, &Task, &Task) -> Result<Task>,
) -> Result<()> {
    let mut tx = pool.begin().await?;
    ...
    for changed in changed_items {
        if let Some(base_snapshot) = pre_tick_snapshot.get(&changed.id) {
            ...
            let (_, transition_id) = update_task_exec(&mut tx, &current, &merged).await?;
        ...
        if changed.id > 0 {
            if let Some(current) = find_by_id_exec(&mut *tx, changed.id).await? {
                let (_, transition_id) = update_task_exec(&mut tx, &current, changed).await?;
                if let Some(transition_id) = transition_id {
                    transition_ids.push(transition_id);
                }
            } else {
