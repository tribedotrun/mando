...
pub(crate) fn ops_log_path() -> PathBuf {
    global_infra::paths::state_dir().join("ops-log.json")
}
...
pub(crate) fn save_ops_log(log: &OpsLog, path: &Path) -> Result<()> {
    global_infra::save_json_file(path, log)?;
    Ok(())
}
...
mod tests {
    ...
    fn begin_op(log: &mut OpsLog, op_type: &str, params: serde_json::Value) -> String {
        let op_id = global_infra::uuid::Uuid::v4().to_string();
        log.entries.push(OpEntry {
            op_id: op_id.clone(),
            op_type: op_type.to_string(),
            params,
            steps_completed: Vec::new(),
            started_at: global_types::now_rfc3339(),
        });
        op_id
    }
