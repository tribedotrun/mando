...
pub use super::tick_spawn::spawn_worker_for_item;
use crate::io::{health_store, task_store::TaskSnapshotJson, task_store::TaskStore};
...
pub(crate) async fn run_captain_tick(
    ...
    let tick_id = global_infra::uuid::Uuid::v4().short();
...
async fn run_captain_tick_inner(
    ...
    let notifier_bus = match bus {
        ...
        None => Arc::new(global_bus::EventBus::new()),
    };
    ...
    let (pre_tick_snapshot, pre_tick_snapshot_json) =
        super::tick_persist::build_pre_tick_snapshots(&items)?;
    ...
    if !dry_run {
        ...
        {
            let changed_items: Vec<crate::Task> = items
                .iter()
                .filter(|item| match TaskSnapshotJson::from_task(item) {
                    Ok(snap) => pre_tick_snapshot_json.get(&item.id).is_none_or(|old| *old != snap),
                    Err(e) => {
