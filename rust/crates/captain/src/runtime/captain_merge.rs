...
pub(crate) async fn apply_merge_result(
    ...
    match result.action.as_str() {
        "merged" => {
            if let Err(e) = lifecycle::apply_transition(item, ItemStatus::Merged) {
                tracing::error!(
                    module = "captain",
                    item_id = item.id,
                    error = %e,
                    "illegal merge-complete transition"
                );
                return;
            }
            ...
            let event = crate::TimelineEvent {
                ...
                data: TimelineEventPayload::Merged {
                    pr: item.pr_number.map(|n| n.to_string()).unwrap_or_default(),
                    source: "captain".to_string(),
                    accepted_by: "captain".to_string(),
                },
            };
...
pub(crate) async fn handle_merge_error(
    ...
    if fail_count >= max_retries {
        ...
        let event = crate::TimelineEvent {
            ...
            data: TimelineEventPayload::CaptainReviewMergeFail {
                trigger: "merge_fail".to_string(),
                fail_count: fail_count as i64,
                error: error.to_string(),
                retries: max_retries as i64,
            },
        };
        match crate::io::queries::tasks::persist_status_transition(
            pool,
        ...
        global_infra::best_effort!(
            timeline_emit::emit_for_task(
                ...
                TimelineEventPayload::CaptainMergeRetry {
                    error: error.to_string(),
                    fail_count: fail_count as i64,
                },
                pool,
            ...
            .await,
            "captain_merge: timeline_emit::emit_for_task( item, &format!('Merge attempt "
        );
    }
}
