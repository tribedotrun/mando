...
use super::captain_review_payload::{
    escalated_payload, read_worktree_head_sha, ship_payload, verdict_payload,
};
...
pub async fn apply_verdict(
    ...
    // auto-merged on a stale high-confidence verdict. Non-ship verdicts
    // and missing worktrees use the `UNKNOWN_SHA` sentinel; the freshness
    // gate rejects it so auto-merge requires a real matching SHA.
    let reviewed_head_sha = if verdict.action == "ship" {
        read_worktree_head_sha(item.worktree.as_deref()).await
    } else {
        super::captain_review_payload::UNKNOWN_SHA.to_string()
    };

    // Verdict carries the full review output including confidence (populated
    // only on ship). Mergeability tick reads `confidence` from the latest
    // CaptainReviewVerdict event to decide auto-merge.
    let action = verdict.action.clone();
    let feedback = verdict.feedback.clone();
    let confidence = verdict.confidence.clone();
    let confidence_reason = verdict.confidence_reason.clone();
    ...
    match verdict.action.as_str() {
        "ship" => {
            ...
            let _ = lifecycle::apply_transition(item, spawn_logic::ship_status(is_no_pr))?;
            item.intervention_count = 0;
            let msg_suffix = if is_no_pr {
                "completed (no PR)"
            } else {
                "ready for review"
            };
            ...
            let confidence_badge = confidence
                .as_deref()
                .map(|c| format!(" (confidence: {c})"))
                .unwrap_or_default();
            let ship_data = ship_payload(
                is_no_pr,
                &action,
                &feedback,
                confidence.as_deref(),
                confidence_reason.as_deref(),
                &reviewed_head_sha,
            );
        ...
        "nudge" => {
            let _ = lifecycle::apply_transition(item, ItemStatus::InProgress)?;
        ...
        "respawn" => {
            ...
            let saved_spawn_fail_count = item.spawn_fail_count;
            ...
            // Fresh respawn — the past spawn failures informed this verdict but
            // shouldn't immediately re-escalate the next attempt on its first
            // stumble. Successful spawn zeroes this field anyway; clearing here
            // just guarantees the retry gets the full MAX_SPAWN_FAILS budget.
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    item.spawn_fail_count = saved_spawn_fail_count;
                    transition_applied = false;
        ...
        "retry_clarifier" => {
            ...
            let _ = lifecycle::apply_transition(item, ItemStatus::New)?;
