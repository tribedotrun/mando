...
mod tests {
    ...
    async fn high_notification_emits_immediately() {
        ...
        let bus_payload = rx.recv().await.unwrap();
        assert!(matches!(
            bus_payload,
            global_bus::BusPayload::Notification(_)
        ));
        let payload = match bus_payload {
    ...
    fn make_cc_result(
        status: global_claude::RateLimitStatus,
        utilization: f64,
    ) -> global_claude::CcResult<serde_json::Value> {
    ...
    fn make_cc_result_opt(
        ...
        global_claude::CcResult {
            ...
            rate_limit: Some(global_claude::RateLimitEvent {
                status,
    ...
    async fn check_rate_limit_first_warning_emits() {
        ...
        let result = make_cc_result(global_claude::RateLimitStatus::AllowedWarning, 0.89);
        n.check_rate_limit(&result, &pool, None).await;

        let payload = match rx.recv().await.unwrap() {
            global_bus::BusPayload::Notification(p) => p,
            other => panic!("unexpected payload: {other:?}"),
        };
    ...
    async fn check_rate_limit_same_tier_suppressed() {
        ...
        let r1 = make_cc_result(global_claude::RateLimitStatus::AllowedWarning, 0.87);
        n.check_rate_limit(&r1, &pool, None).await;
        // Consume first notification.
        rx.recv().await.unwrap();

        // Second at same tier → suppressed.
        let r2 = make_cc_result(global_claude::RateLimitStatus::AllowedWarning, 0.89);
    ...
    async fn check_rate_limit_rejected_always_fires() {
        ...
        let r1 = make_cc_result(global_claude::RateLimitStatus::Rejected, 1.0);
        n.check_rate_limit(&r1, &pool, None).await;
        let p = match rx.recv().await.unwrap() {
            global_bus::BusPayload::Notification(p) => p,
            other => panic!("unexpected payload: {other:?}"),
        };
        ...
        let r2 = make_cc_result(global_claude::RateLimitStatus::Rejected, 1.0);
        n.check_rate_limit(&r2, &pool, None).await;
        let p2 = match rx.recv().await.unwrap() {
            global_bus::BusPayload::Notification(p) => p,
            other => panic!("unexpected payload: {other:?}"),
        };
    ...
    async fn check_rate_limit_allowed_clears_tiers() {
        ...
        // Recovery.
        let r2 = make_cc_result(global_claude::RateLimitStatus::Allowed, 0.50);
        n.check_rate_limit(&r2, &pool, None).await;

        // Same tier fires again after recovery.
        let r3 = make_cc_result(global_claude::RateLimitStatus::AllowedWarning, 0.87);
    ...
    async fn check_rate_limit_missing_utilization_always_fires() {
        ...
        let r1 = make_cc_result_opt(global_claude::RateLimitStatus::AllowedWarning, None);
        n.check_rate_limit(&r1, &pool, None).await;
        ...
        let r2 = make_cc_result_opt(global_claude::RateLimitStatus::AllowedWarning, None);
        n.check_rate_limit(&r2, &pool, None).await;
    ...
    async fn check_rate_limit_includes_credential_label() {
        let pool = test_pool().await;
        let cred_id = settings::credentials::insert(&pool, "team-alpha", "tok_test", None)
            .await
            .unwrap();
        ...
        let r1 = make_cc_result(global_claude::RateLimitStatus::AllowedWarning, 0.92);
        ...
        let r3 = make_cc_result_opt(global_claude::RateLimitStatus::AllowedWarning, None);
        n.check_rate_limit(&r3, &pool, Some(cred_id)).await;
        let p3 = match rx.recv().await.unwrap() {
            global_bus::BusPayload::Notification(p) => p,
            other => panic!("unexpected payload: {other:?}"),
        };
        assert!(
