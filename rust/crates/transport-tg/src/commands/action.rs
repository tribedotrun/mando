...
async fn show_picker(bot: &mut TelegramBot, chat_id: &str) -> Result<()> {
    ...
    for (i, it) in display.iter().enumerate() {
        ...
        buttons.push(vec![InlineKeyboardButton {
            ...
            callback_data: Some(format!("act:pick:{action_id}:{i}")),
            url: None,
        }]);
    }

    // Two columns when many tasks
    let keyboard_buttons: Vec<Vec<InlineKeyboardButton>> = if buttons.len() > 4 {
        buttons
            .chunks(2)
    ...
    let mut kb_rows = keyboard_buttons;
    kb_rows.push(vec![InlineKeyboardButton {
        text: "\u{274c} Cancel".into(),
        callback_data: Some(format!("act:cancel:{action_id}")),
        url: None,
    }]);
    ...
    bot.api()
        .send_message(
            ...
            &lines.join("\n"),
            Some("HTML"),
            Some(api_types::TelegramReplyMarkup::InlineKeyboard { rows: kb_rows }),
            true,
        )
        .await?;
...
pub(crate) fn action_buttons(
    ...
    status: ItemStatus,
    has_pr: bool,
) -> Vec<Vec<api_types::InlineKeyboardButton>> {
    use api_types::InlineKeyboardButton;
        ...
        .map(|chunk| {
            chunk
                ...
                .map(|(label, action)| InlineKeyboardButton {
                    ...
                    callback_data: Some(format!("act:do:{task_id}:{action}")),
                    url: None,
                })
                .collect()
        })
