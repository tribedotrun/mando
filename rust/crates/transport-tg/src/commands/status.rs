...
pub async fn handle(bot: &TelegramBot, chat_id: &str, args: &str) -> Result<()> {
    ...
    let mut action_buttons: Vec<Vec<InlineKeyboardButton>> = Vec::new();
                ...
                match status {
                    ItemStatus::AwaitingReview => {
                        if let Some(pr_num) = item.pr_number {
                            action_buttons.push(vec![InlineKeyboardButton {
                                text: format!("Merge PR #{pr_num}"),
                                callback_data: Some(format!("merge:{id}")),
                                url: None,
                            }]);
                        } else {
                            action_buttons.push(vec![InlineKeyboardButton {
                                text: format!("\u{2705} Accept \u{2014} {title_short}"),
                                callback_data: Some(format!("accept:{id}")),
                                url: None,
                            }]);
                        }
                    }
                    ItemStatus::NeedsClarification => {
                        action_buttons.push(vec![InlineKeyboardButton {
                            ...
                            callback_data: Some(format!("answer:{id}")),
                            url: None,
                        }]);
                    }
                    ItemStatus::Errored => {
                        action_buttons.push(vec![InlineKeyboardButton {
                            ...
                            callback_data: Some(format!("retry:{id}")),
                            url: None,
                        }]);
                    }
                    _ => {}
    ...
    for row in capped.chunks(5) {
        let btns: Vec<InlineKeyboardButton> = row
            .iter()
            .map(|id| InlineKeyboardButton {
                text: format!("#{id}"),
                callback_data: Some(format!("view:{id}")),
                url: None,
            })
            .collect();
        action_buttons.push(btns);
    }
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        let markup = if i == 0 && !action_buttons.is_empty() {
            Some(api_types::TelegramReplyMarkup::InlineKeyboard {
                rows: action_buttons.clone(),
            })
        } else {
            None
        };
