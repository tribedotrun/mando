...
pub(crate) async fn add_and_track(
        ...
        Err(e) => {
            ...
            global_infra::best_effort!(
                bot.api
                    ...
                    .await,
                "helpers: bot .api .edit_message_text(chat_id, message_id, &msg, Some("
            );
            return Ok(());
        }
    };
    ...
    global_infra::best_effort!(
        bot.api
            ...
            .await,
        "helpers: bot .api .edit_message_text(chat_id, message_id, &processing"
    );
...
pub(crate) async fn handle_implicit_addlink(
    ...
    for url in &urls {
            ...
            .await
        {
            ...
            Err(e) => {
                warn!(module = "transport-tg", url = %url, error = %e, "implicit addlink failed");
    ...
    global_infra::best_effort!(
        bot.api
            ...
            .await,
        "helpers: bot .api .edit_message_text(chat_id, message_id, &lines.join"
    );
    Ok(())
}
