...
fn url_btn(text: impl Into<String>, url: impl Into<String>) -> InlineKeyboardButton {
    InlineKeyboardButton {
        text: text.into(),
        callback_data: None,
        url: Some(url.into()),
...
/// [⭐ Save] [📦 Archive] [⚙️ Act] [🗑]
/// ```
pub fn swipe_card_kb(item_id: i64, telegraph_url: Option<&str>) -> TelegramReplyMarkup {
    let mut top_row: Vec<InlineKeyboardButton> = Vec::new();
    if let Some(url) = telegraph_url.filter(|u| !u.is_empty()) {
        top_row.push(url_btn("\u{1f4d6} Read", url));
    } else {
        top_row.push(callback_btn("\u{1f4d6} Read", format!("dg:read:{item_id}")));
    }
    top_row.push(callback_btn("\u{1f4ac} Ask", format!("dg:ask:{item_id}")));
    top_row.push(callback_btn("Next \u{25b6}", format!("dg:next:{item_id}")));

    let action_row = vec![
        callback_btn("\u{2b50} Save", format!("dg:save:{item_id}")),
        callback_btn("\u{1f4e6} Archive", format!("dg:archive:{item_id}")),
        callback_btn("\u{2699}\u{fe0f} Act", format!("dg:act:{item_id}")),
        callback_btn("\u{1f5d1}", format!("dg:rm:{item_id}")),
    ];
    TelegramReplyMarkup::InlineKeyboard {
        rows: vec![top_row, action_row],
    }
}
...
pub fn list_kb(
    ...
    for (chunk_idx, chunk) in item_ids.chunks(items_per_row).enumerate() {
        let row: Vec<InlineKeyboardButton> = chunk
            ...
            .map(|(i, id)| {
                ...
                callback_btn(format!("{pos}"), format!("dg:show:{id}"))
            })
            .collect();
        rows.push(row);
    }
    ...
    if rows.is_empty() {
        ...
        Some(TelegramReplyMarkup::InlineKeyboard { rows })
    }
}
...
pub fn telegraph_read_kb(item_id: i64, url: &str) -> TelegramReplyMarkup {
    let link_row = vec![url_btn("\u{1f4d6} Read on Telegraph", url)];
    let action_row = vec![
        callback_btn("\u{25c0} Summary", format!("dg:show:{item_id}")),
        callback_btn("\u{2b50} Save", format!("dg:save:{item_id}")),
        callback_btn("\u{1f4e6} Archive", format!("dg:archive:{item_id}")),
    ];
    TelegramReplyMarkup::InlineKeyboard {
        rows: vec![link_row, action_row],
    }
}
...
pub fn act_project_picker_kb(item_id: i64, project_names: &[String]) -> TelegramReplyMarkup {
    let buttons: Vec<InlineKeyboardButton> = project_names
        .iter()
        .map(|name| callback_btn(name, format!("dg:actpick:{item_id}:{name}")))
        .collect();
    TelegramReplyMarkup::InlineKeyboard {
        rows: vec![buttons],
    }
...
///
/// `[▶ Skip — use default]`
pub fn act_prompt_kb(item_id: i64) -> TelegramReplyMarkup {
    TelegramReplyMarkup::InlineKeyboard {
        rows: vec![vec![callback_btn(
            "\u{25b6} Skip \u{2014} use default",
            format!("dg:actskip:{item_id}"),
        )]],
    }
}
...
///
/// `[◀ Summary] [⏹ End session]`
pub fn qa_session_kb(item_id: i64) -> TelegramReplyMarkup {
    TelegramReplyMarkup::InlineKeyboard {
        rows: vec![vec![
            callback_btn("\u{25c0} Summary", format!("dg:show:{item_id}")),
            callback_btn("\u{23f9} End session", format!("dg:endqa:{item_id}")),
        ]],
    }
}
...
mod tests {
    use super::*;
    use crate::telegram_markup::to_bot_api_json;
    use serde_json::json;

    /// Extract the `inline_keyboard` rows from a Bot-API-shaped Value,
    /// failing the test with a clear structural message if the shape is wrong.
    ...
    fn swipe_card_kb_has_action_row() {
        let kb = to_bot_api_json(&swipe_card_kb(42, None));
        let rows = kb_rows(&kb);
    ...
    fn swipe_card_kb_youtube_telegraph() {
        let kb = to_bot_api_json(&swipe_card_kb(42, Some("https://telegra.ph/Test-42")));
        let rows = kb_rows(&kb);
        let top_row = row_array(&rows[0], "swipe card top row");
        assert_eq!(top_row.len(), 3);
        assert_eq!(
    ...
    fn qa_session_kb_has_end_button() {
        let kb = to_bot_api_json(&qa_session_kb(42));
        let rows = kb_rows(&kb);
    ...
    fn list_kb_items_only_no_pagination() {
        let kb = to_bot_api_json(&list_kb(&[10, 20, 30], 0, 1, "all", "dg:page", 5, 0).unwrap());
        let rows = kb_rows(&kb);
    ...
    fn list_kb_with_pagination() {
        let kb = to_bot_api_json(&list_kb(&[1, 2, 3], 0, 3, "saved", "dg:page", 5, 0).unwrap());
        let rows = kb_rows(&kb);
    ...
    fn list_kb_chunked_rows() {
        ...
        let kb = to_bot_api_json(&list_kb(&ids, 1, 3, "all", "dg:cpage", 5, 5).unwrap());
        let rows = kb_rows(&kb);
