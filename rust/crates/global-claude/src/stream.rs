...
pub fn write_error_result(stream_path: &Path, error: &str) {
    ...
    if let Some(parent) = stream_path.parent() {
        if let Err(e) = std::fs::create_dir_all(parent) {
            tracing::warn!(
                module = "global-claude-stream",
                parent = %parent.display(),
                %e,
                "failed to pre-create stream directory before error-result write",
            );
        }
    }
        ...
        .open(stream_path)
    {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(module = "global-claude-stream", %e, "failed to write error result to stream");
            return;
        }
    };
    ...
    if !locked {
        tracing::warn!(
            module = "global-claude-stream", path = %stream_path.display(),
            "failed to acquire exclusive flock on stream — writing without serialization"
        );
    }
    if let Err(e) = writeln!(file, "{}", line) {
        tracing::warn!(module = "global-claude-stream", %e, path = %stream_path.display(), "failed to write error result line to stream");
    }
    if locked {
